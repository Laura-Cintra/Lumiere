...
export default function Games() {
  ...
  return (
    <main>
      ...
      <ContainerDestaqueGame>
        ...
        <div className="conteudo-games">
          <div className="explicacao-itens">
            <p>
              Aqui na Lumière você ganha pontos por atos sustentáveis. Além de aprender a ser mais consciente e
              melhorar o consumo na sua casa, você também pode ganhar pontos!
            </p>
            <Image
              src={btn}
              alt="seta para ir ao conteúdo"
              className="btn"
              onClick={() => {
      ...
      <CategoriaContainer>
        <CategoriaButton isSelected={categoriaSelecionada === "quiz"} onClick={() => setCategoriaSelecionada("quiz")}>
          Quizzes
        </CategoriaButton>
        <CategoriaButton isSelected={categoriaSelecionada === "dicas"} onClick={() => setCategoriaSelecionada("dicas")}>
          Dicas
        </CategoriaButton>
        <CategoriaButton isSelected={categoriaSelecionada === null} onClick={() => setCategoriaSelecionada(null)}>
          Todos
        </CategoriaButton>
      </CategoriaContainer>
      ...
      {/* Modal do quiz */}
      {showModal && quizId && (
        <ModalQuiz quizId={quizId} onClose={handleCloseModal} idUsuario={user?.id_usuario as number} />
      )}
    </main>
  );
}
